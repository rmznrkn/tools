...
void fout_field(char *ident, Field *field, FILE* fp) {
   ...
   if (length && name) {
      if (compare_incase_sensitive("variable", length) == 0)
         fprintf(fp, "%schar ac_%s [1];//", ident, clear_whitespacesWithCamelNotation(name));
      else if (compare_incase_sensitive("char", length) != 0)
...
int fout_field_is_variable(char *ident, Field *field, FILE* fp) {
   if (field == NULL )
      return 0;
...
void fout_class_addfields(char *ident, Field *field, FILE* fp) {
   ...
   if (length && name) {
      ...
      if (compare_incase_sensitive("variable", length) == 0) {
         ...
         fprintf(fp, "%schar m_ac%s[TSMR_MAXBUFSIZE];\n", ident,
...
void fout_message2(Message *message, FILE *fph, FILE *fpcpp, char *namespace, FILE *fpDef,
      FILE *fpCase) {
#define LBUF_LEN 256
   static int lcnt = 0;
   static int rdcnt = 0;
   ...
   int isSructEmpty = 0;
   int isMsgTypeNotOk;
   int isVariableSize = 0;
   char variableName[LBUF_LEN];
   ...
   while (field != NULL ) {
      if (fout_field_is_variable("  ", field, fph)) {
         isVariableSize = 1;
         break;
      }
      field = field->next;
   ...
   if (isVariableSize)
      printf("variables class type: %s", classTypeName);
...
//1
   ...
   if (message->firstField)
      fprintf(fph, "\n"
            ...
            "    //}\n"
            "    this->set(m.m_dataBuffer, m.m_dataLen);\n"
            "  }\n", structName, structName);
         ...
         if (compare_incase_sensitive("request", requestReplyType) == 0) {
            fprintf(fph, "  virtual bool isQueryRequest(){\n"
                  ...
                  "  }\n");
                        ...
                        fprintf(fpCase, "    case %s:\n", messageType);
                        fprintf(fpCase, "       %sReply.set(reply->getDataRef(), reply->getDataLength());\n",
                              tolover_string(variableName, classTypeName));

                        fprintf(fpCase, "       host->m_hostToFixMsgConverter->toFix(%sReply, session->m_sessionSpec.m_userID);\n", tolover_string(variableName, classTypeName));
                        fprintf(fpCase, "       break;\n");

                        fprintf(fpDef, "  %s(),\n",tolover_string(variableName, classTypeName));
                        /*fprintf(fpDef, "  VIOP::MSG::%s::%sCLASSES::%-50s %s;\n", nameSpace, UYARI,
                              classTypeName, tolover_string(variableName, classTypeName));
                              */
...
/*
            ...
            fprintf(fpDef, "  %sReply(),\n",tolover_string(variableName, classTypeName));
            */
/*
            fprintf(fpDef, "  void HostToFixMessageConverter::toFix(const VIOP::MSG::%s::%sCLASSES::%s& %sReply, std::string userID)\n"
                  "  {\n"
                  "    #if DEBUG\n"
                  "    std::cout << __FILE__ << \":\" << __FUNCTION__ << \":\" << __LINE__ << \"=>\" << \"%s.process()\" std::endl;\n"
                  "    #endif\n"
                  "    //FIX::Message msg;\n"
                  "    if(m_replyInterface){\n"
                  "    //m_replyInterface->sendExecutionReport(msg, userID);\n"
                  "    }\n"
                  "  }\n", nameSpace, UYARI,
                              classTypeName, tolover_string(variableName, classTypeName),tolover_string(variableName, classTypeName));
*/

  //          fprintf(fpDef, "  void toFix(const VIOP::MSG::%s::%sCLASSES::%s& %sReply, std::string userID);\n", nameSpace, UYARI,
  //                                        classTypeName, tolover_string(variableName, classTypeName));

            //fprintf(fpDef, "  VIOP::MSG::%s::%sCLASSES::%-50s %sReply;\n", nameSpace, UYARI,
            //      classTypeName, tolover_string(variableName, classTypeName));

         }
      }
   }
   if (compare_incase_sensitive("transactionreply", nameSpace) == 0) {
      fprintf(fpCase, "    case %s:\n", messageType);
      fprintf(fpCase, "       %sReply.set(reply->getDataRef(), reply->getDataLength());\n",
            tolover_string(variableName, classTypeName));
      fprintf(fpCase, "       break;\n");

      fprintf(fpDef, "  MSG::%s::%sCLASSES::%-50s %sReply;\n", nameSpace, UYARI,
            classTypeName, tolover_string(variableName, classTypeName));
   }
...
void fout_message_list(MessageList *ml, FILE *fph, FILE *fpcpp, char *nspace, FILE *defFile,
      FILE *caseFile) {
   if (ml == NULL )
      return;
   Message *message = ml->firstMessage;
   while (message != NULL ) {
      fout_message2(message, fph, fpcpp, nspace, defFile, caseFile);
      message = message->next;
      //break;
   }
}
...
int main(int argc, char *argv[]) {
...
#if 0
   char signalName[100];
   char signals[][100] = {"SIGHUP","SIGINT", "SIGQUIT",     "SIGILL","SIGTRAP",
    "SIGABRT","SIGEMT","SIGFPE","SIGKILL","SIGBUS",
   "SIGSEGV","SIGSYS","SIGPIPE","SIGALRM","SIGTERM",
   "SIGUSR1","SIGUSR2","SIGCHLD","SIGPWR","SIGVTALRM",
   "SIGPROF","SIGIO","SIGWINCH","SIGSTOP","SIGTSTP",
   "SIGCONT","SIGTTIN","SIGTTOU","SIGURG","SIGLOST",
   "SIGXCPU","SIGXFSZ","SIGCANCEL"};

   for(i = 0; i < sizeof(signals)/100; i++){
      tolover_string(signalName, signals[i]);
      signalName[0] = toupper(signalName[0]);
      signalName[3] = toupper(signalName[3]);
      printf("  static void handle%s(int signum){\n"
             "      WTRACE(\"%s catched!\");\n"
             "      exit(signum);\n"
             "  }\n", signalName,signals[i]);
   }
   printf(" static void setupSignals(){\n");
   for(i = 0; i < sizeof(signals)/100; i++){
         tolover_string(signalName, signals[i]);
         signalName[0] = toupper(signalName[0]);
         signalName[3] = toupper(signalName[3]);
         printf("  if(signal((int)%s, handle%s) == SIG_ERR){\n"
                "      ETRACE(\"%s setup failed!\");\n"
                "  }\n",signals[i], signalName, signals[i]);
      }
   printf("  }\n");
   return 0;
   //"SIGRTMIN","SIGRTMIN+1","SIGRTMIN+2","SIGRTMIN+3","SIGRTMAX-3","SIGRTMAX-2","SIGRTMAX-1","SIGRTMAX"
#endif
   ...
   deffFile = fopen("classdefs.c", "w");
   caseFile = fopen("classcases.c", "w");

   //openfile(&hTrsnReply, 1, 0, "transactionReplyMessageDefs", "transactionReply", "FIELDDEFS",
   //      "MSGTYPEDEFS", NULL );
   //openfile(&cppTrsnReply, 0, 0, "transactionReplyMessageDefs", "transactionReply", "FIELDDEFS",
   //      "MSGTYPEDEFS", NULL );
...
//fout_message_list(ml,stdout, stdout);
      fout_message_list(ml, hOpr, cppOpr, "Operator", deffFile, caseFile);
      printf("OperatorFinish\n");
      fout_message_list(ml, hTrans, cppTrans, "Transaction", deffFile, caseFile);
      printf("TransactionFinish\n");
      //fout_message_list(ml, hTrsnReply, cppTrsnReply, "TransactionReply",deffFile, caseFile);
      fout_message_list(ml, hQuery, cppQuery, "Query", deffFile, caseFile);
      printf("QueryFinish\n");
   ...
   closefile(cppTrans, 0, "transactionMessageDefs", "transaction", "FIELDDEFS", "MSGTYPEDEFS",
         NULL );
   //closefile(hTrsnReply, 1, "transactionReplyMessageDefs", "transactionReply", "FIELDDEFS",
   //      "MSGTYPEDEFS", NULL );
   //closefile(cppTrsnReply, 0, "transactionReplyMessageDefs", "transactionReply", "FIELDDEFS",
   //      "MSGTYPEDEFS", NULL );
